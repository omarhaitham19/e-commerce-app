            ...
            <div class="row">
                <div class="col-md-12">
                  @include('error')
                              ...
                              <?php $totalPrice = 0 ?>
                                ...
                                <tr>
                                  ...
                                  <td>{{$cart->product->name}}</td>
                                    <td>{{$cart->quantity}}</td>
                                    <td>{{$cart->price}}L.E</td>
                                    <td><img src="{{ asset('storage/'. $cart->product->image ) }}" alt="Product Image" style="max-width: 20%;"></td>
                                    <td>
                                      <form action="{{url("remove_cart/$cart->id")}}" method="post">
                                        @csrf
                                        <button type="submit" class="btn btn-danger remove-button" onclick="return confirm('Are you sure to remove this item')">
                                          Remove</button>
                                      </form>
                                    </td> 
                                  </tr>
                        ...
                        <div class="proceed-to-order-container">
                        <button type="submit" class="proceed-to-order-btn">cash on delivery</button>
                        </div>
                      </form>
                      {{-- <form action="{{url('stripe')}}" method="GET">  // uncomment this part when you put your stripe credentials in .env file
                        @csrf
                        <input type="hidden" name="totalPrice" value="{{$totalPrice}}">
                        <div class="proceed-to-order-container">
                        <button type="submit" class="proceed-to-order-btn">Pay by card</button>
                        </div>
                      </form>  --}}
                    @endif
                </div>
